...
async function addSave() {
    // var yes = confirm("確定要儲存設定嗎?");
    var yes = await customConfirm("確定要儲存設定嗎?");
    if (yes) {
        // 使用 Promise.all 以確保所有 add() 調用完成後再繼續
        await Promise.all(FUN_SID_BIND_LIST.map(async (item) => {
            add(item, masterSID);
        }));
        
        await customAlertSuccess('加入群組成功!');
        getBindDetailTable();
        getUnbindDetailTable();
...
async function delSave() {
    ...
    if (yes) {
        // 使用 Promise.all 以確保所有 add() 調用完成後再繼續
        await Promise.all(FUN_SID_DEL_LIST.map(async (item) => {
            del(item,masterSID);
        }));
        
        await customAlertSuccess('移出群組成功!');
        getBindDetailTable();
        getUnbindDetailTable();
    }
}
